        ...
        <div class="card-body">
            ...
            <div class="progress mt-3" style="height: 2px;">
                <div class="progress-bar bg-danger" role="progressbar" aria-valuenow="62" aria-valuemin="0"
                    aria-valuemax="100" style="width: {{ (int) $progress_plan }}%;"></div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            ...
            <form class="form-prevent" action="#" method="post" enctype="multipart/form-data" autocomplete="off">
                ...
                <div class="table-responsive">
                    ...
                    <table
                        class="myDataTable table table-hover align-middle mb-0 nowrap dataTable no-footer dtr-inline collapsed">
                        <tr>
                            ...
                            @if ($supervisi->supervisi_project->status_plan == 0)
                                <th width="10%" rowspan="2"
                                    class="text-center bg-light-info active color-palette">
                                    ACTION PLAN
                                </th>
                            @endif
                                ...
                                @if ($supervisi->supervisi_project->status_plan == 0)
                                    <td>
                                        ...
                                        @if ($list->activity_id >= 1 && $list->activity_id <= 2)
                                            @if ($list->plan_finish)
                                                <a data-bs-toggle="modal" data-bs-target="#createDate"
                                                    data-id="{{ $list->id }}"
                                                    data-activityId="{{ $list->activity_id }}"
                                                    data-name="{{ $list->list_activity }}"
                                                    data-start="{{ $list->plan_start }}" id="cek"
                                                    class="btn btn-light get-id">&nbsp;&nbsp;Edit Plan</a>
                                            @else
                                                <a data-bs-toggle="modal" data-bs-target="#createDate"
                                                    data-id="{{ $list->id }}"
                                                    data-activityId="{{ $list->activity_id }}"
                                                    data-name="{{ $list->list_activity }}"
                                                    data-start="{{ $supervisi->supervisi_project->start_date }}"
                                        ...
                                        @if (($list->activity_id >= 3 && $list->activity_id <= 9) || ($list->activity_id == 0 && $list->category_id == '002'))
                                            @if ($list->plan_finish)
                                                <a data-bs-toggle="modal" data-bs-target="#createDate"
                                                    data-id="{{ $list->id }}"
                                                    data-activityId="{{ $list->activity_id }}"
                                            ...
                                            @else
                                                @if ($cek_preparing->plan_finish)
                                                    <a data-bs-toggle="modal" data-bs-target="#createDate"
                                                        ...
                                                        Plan</a>
                                                @else
                                                    <a data-bs-target="#createDate" data-id="{{ $list->id }}"
                                                        data-activityId="{{ $list->activity_id }}"
                                                        data-name="{{ $list->list_activity }}"
                                                        data-start="{{ $cek_preparing->plan_finish }}" id="cek"
                                                        class="btn btn-primary get-id">&nbsp;&nbsp;Add
                                        ...
                                        @if (
                                            ($list->activity_id >= 10 && $list->activity_id <= 19 && $cek_all_delivery == $cek_all_delivery_finish) ||
                                                ($list->activity_id == 0 && $list->category_id == '003'))
                                            @if ($list->plan_finish)
                                                <a data-bs-toggle="modal" data-bs-target="#createDate"
                                                    data-id="{{ $list->id }}"
                                                    data-activityId="{{ $list->activity_id }}"
...
<script>
    $(document).ready(function() {
        ...
        $(".get-id").click(function() {
            ...
            if (activity_id == 3 || activity_id == 10 || activity_id == 20 || activity_id == 21 ||
                activity_id == 22 || activity_id == 23) {
